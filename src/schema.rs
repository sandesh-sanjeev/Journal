...
use crate::log::{Error as LogError, Log, Result as LogResult};
use std::borrow::Cow;
...
impl<'a> LogSerializer<'a, V1> for Log<'a> {
    ...
    fn write(&self, dst: &mut Vec<u8>) {
        ...
        dst.extend_from_slice(&self.seq_no.to_be_bytes());
        dst.extend_from_slice(&self.prev_seq_no.to_be_bytes());
        dst.extend_from_slice(&(self.data.len() as u32).to_be_bytes());
        ...
        let hash = V1::hash(&dst[start_len..]);
        dst.extend_from_slice(&hash.to_be_bytes());
    }
